...
本章实现了一个最简单的 RISC-V S 态裸机程序（tg-rcore-tutorial-ch1-uart），展示操作系统的最小执行环境。程序在 QEMU 模拟的 RISC-V 64 硬件上运行，不依赖 OpenSBI 或 RustSBI，通过 `-bios none` 模式直接启动，使用 S-Mode UART 轮询驱动打印 `Hello, world!` 后关机。
...
- 学：读本文件，了解相关OS知识，在某个开发环境（在线或本地）中正确编译运行 tg-rcore-tutorial-ch1-uart。
- 教：分析并改进 tg-rcore-tutorial-ch1-uart 与 tg-rcore-tutorial-uart 的文档和代码，让自己更高效地完成本章学习。
- 用：基于 tg-rcore-tutorial-ch1-uart 的源代码，用 gpu framebuffer 显示以代码中的数组表示的七巧板图形信息，形成七巧板构成的“O”和“S”图案。[demo](https://github.com/rcore-os/tg-rcore-tutorial-game-demo/blob/main/ch1-tangram.png)
...
├── README.md           # 本文档
├── src/
│   └── main.rs         # 内核源码：入口、主函数、panic 处理
└── tg-rcore-tutorial-uart/
    ├── Cargo.toml      # UART 组件 crate 配置
    └── src/lib.rs      # UART16550 轮询发送实现
```
...
| 2 | `rust_main` | 最小执行环境中，`uart_puts` 和 `shutdown` 如何构成完整闭环？ |
| 3 | `panic_handler` | `#![no_std]` 下发生异常时，系统如何收口与退出？ |

配套建议：阅读 `tg-rcore-tutorial-uart/src/lib.rs` 与 `tg-rcore-tutorial-sbi/src/lib.rs`，理解 UART 轮询输出与关机路径。

## DoD 验收标准（本章完成判据）

- [ ] 能在 `tg-rcore-tutorial-ch1-uart` 目录执行 `cargo run`，看到 `Hello, world!` 并正常关机退出
- [ ] 能解释 `#![no_std]` 与 `#![no_main]` 在裸机实验中的必要性
- [ ] 能从 `src/main.rs` 说明 `_start -> rust_main -> panic_handler` 的控制流
- [ ] 能说明 `tg-rcore-tutorial-uart` 与 `tg-rcore-tutorial-sbi` 在本章承担的最小职责

## 概念-源码-测试三联表

| 核心概念 | 源码入口 | 自测方式（命令/现象） |
|---|---|---|
| 裸机入口与手动设栈 | `tg-rcore-tutorial-ch1-uart/src/main.rs` 的 `_start` | `cargo run` 可启动且无运行时依赖报错 |
| UART 轮询输出 | `tg-rcore-tutorial-ch1-uart/src/main.rs` 的 `rust_main`；`tg-rcore-tutorial-uart/src/lib.rs` | 看到串口输出后正常关机 |
| 无标准库异常处理 | `tg-rcore-tutorial-ch1-uart/src/main.rs` 的 `panic_handler` | 人为触发 panic 时可打印信息并异常关机 |
...
由于 tg-rcore-tutorial-ch1-uart 是面向 RISC-V 64 裸机平台的程序，需要添加对应的编译目标：
...
tg-rcore-tutorial-ch1-uart 在 QEMU 模拟的 RISC-V 64 虚拟机上运行，需要安装 `qemu-system-riscv64`（建议版本 >= 7.0）。
...
只获取本实验
```bash
cargo clone tg-rcore-tutorial-ch1-uart
...
在 `tg-rcore-tutorial-ch1-uart` 目录下执行：
...
编译成功后，可执行文件位于 `target/riscv64gc-unknown-none-elf/debug/tg-rcore-tutorial-ch1-uart`。
...
qemu-system-riscv64 \
    ...
    -nographic \
    -bios none \
    -kernel target/riscv64gc-unknown-none-elf/debug/tg-rcore-tutorial-ch1-uart
```
...
| `-bios none` | 不加载任何 BIOS/SBI 固件，tg-rcore-tutorial-ch1-uart 通过依赖 `tg-rcore-tutorial-sbi` 自带 M-mode 启动代码 |
...
以下内容帮助你理解 tg-rcore-tutorial-ch1-uart 代码背后的操作系统原理。
...
**tg-rcore-tutorial-ch1-uart 做了什么？** 它跳过了标准库和操作系统内核，在 S-Mode 直接通过 UART MMIO 轮询输出字符，并通过 SBI 执行关机。这就是“最小执行环境”的含义。

### 3.2 移除标准库依赖

要让程序在裸机上运行，首先需要摆脱对操作系统的依赖。Rust 标准库 `std` 依赖操作系统提供的系统调用（如文件 I/O、内存分配、线程等），在没有操作系统的裸机上无法使用。

tg-rcore-tutorial-ch1-uart 在 `src/main.rs` 的开头使用了两个关键的属性标记：
...
标准库提供了 panic 时打印错误信息并终止程序的功能。使用 `#![no_std]` 后，需要自己实现 panic 处理函数。tg-rcore-tutorial-ch1-uart 中的实现是直接调用 SBI 关机：
...
tg-rcore-tutorial-ch1-uart 采用 **nobios 模式**（`-bios none`），不依赖外部 SBI 固件，而是在 `tg-rcore-tutorial-sbi` 库中自带了一个最小的 M-mode 启动代码。启动流程如下：
...
跳转到 0x80000000（M-mode 入口，tg-rcore-tutorial-sbi 的 _m_start）
    ...
    │  ── 设置中断委托、PMP 等
    ▼
跳转到 0x80200000（S-mode 入口，tg-rcore-tutorial-ch1-uart 的 _start）
    │  ── 设置栈指针 sp
    ▼
跳转到 rust_main()
    │  ── 通过 UART 轮询输出 "Hello, world!"
    │  ── 调用 SBI shutdown 关机
    ▼
QEMU 退出
```
...
在本实验中，字符输出改由 `tg-rcore-tutorial-uart` 完成，SBI 主要用于关机：

| 函数 | 说明 |
|------|------|
| `shutdown(fail)` | 关闭虚拟机（`fail=false` 正常关机，`fail=true` 异常关机） |

`rust_main` 的实现非常简洁——调用 UART 输出接口打印 "Hello, world!\n"，然后关机：
...
传统方案（如 rCore-Tutorial 旧版）使用外部 SBI 固件（如 RustSBI），需要将 SBI 固件和内核分别加载。tg-rcore-tutorial-ch1-uart 采用 `tg-rcore-tutorial-sbi` 的 `nobios` 特性，将 M-mode 启动代码直接编译进同一个 ELF 文件中，因此可以用 `-bios none -kernel` 的方式一步加载，简化了启动流程。
...
```toml
[package]
name = "tg-rcore-tutorial-ch1-uart"
edition = "2024"
# ...
...
[dependencies]
tg-rcore-tutorial-sbi = { version = "0.4.8", features = ["nobios"] }
...
- `tg-rcore-tutorial-uart` 提供 S-Mode UART 轮询发送能力，作为字符输出路径
...
**依赖引入（第 27-30 行）：**
- `use tg_rcore_tutorial_uart::uart_puts` 引入 UART 输出接口
- `use tg_sbi::shutdown` 引入关机接口

**入口函数 `_start`（第 32-60 行）：**
...
**主函数 `rust_main`（第 66-69 行）：**
- 调用 `uart_puts` 输出 "Hello, world!\n"
- 调用 `shutdown(false)` 正常关机

**panic 处理（第 71-77 行）：**
- 发生 panic 时调用 `shutdown(true)` 以异常方式关机

**非 RISC-V 占位模块 `stub`（第 79-100 行）：**
...
4. **认识了 RISC-V 特权级与最小固件接口**：M-mode / S-mode / U-mode 的层次关系，以及 `ecall` 指令如何跨越特权级
...
| `tg-rcore-tutorial-uart` | UART16550 轮询驱动库，提供 S-Mode 串口字符输出 |
...
|:-----|:------------|:---------|:---------------|
| **tg-rcore-tutorial-ch1-uart** | 应用程序执行环境<br>裸机编程（Bare-metal）<br>UART16550 轮询驱动<br>RISC-V 特权级（M/S-mode）<br>链接脚本（Linker Script）<br>内存布局（Memory Layout）<br>Panic 处理 | 最小 S-mode 裸机程序<br>QEMU 直接启动（无 OpenSBI）<br>通过 UART 轮询打印 "Hello, world!" 并关机<br>演示最基本的 OS 执行环境 | tg-rcore-tutorial-sbi<br>tg-rcore-tutorial-uart |
